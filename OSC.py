...
v0.3.7 - 11 December 2020
	Added python 3 support 
	All string are now bytes
...
##MODIF JYP n20201211 remove string
import math, re, socket, select, struct, sys, threading, time, types, array, errno, inspect
...
class OSCMessage(object):
	...
	def getBinary(self):
		...
		##MODIF JYP 20201211
		binary += self.message.encode('utf8')
...
def OSCString(next):
	...
	##MODIF JYP 20201211
	return struct.pack(">%ds" % (OSCstringLength), bytes(next,'utf8'))
...
def decodeOSC(data):
	...
	"""
	## MODIF 20201211 string to bytes
	table = {b"i":_readInt, b"f":_readFloat, b"s":_readString, b"b":_readBlob, b"d":_readDouble, b"t":_readTimeTag}
	decoded = []	
	...
	if address == b"#bundle":
		...
		if typetags.startswith(b","):			
			value, rest = table[typetags[1:]](rest)
...
def getRegEx(pattern):
	...
	"""
	## MODIF 2020112 string to binary
	# Translate OSC-address syntax to python 're' syntax	
	pattern = pattern.replace(b".", b"\.")		# first, escape all '.'s in the pattern.	
	pattern = pattern.replace(b"(", b"\(")		# escape all '('s.
	pattern = pattern.replace(b")", b"\)")		# escape all ')'s.
	pattern = pattern.replace(b"*", b".*")		# replace a '*' by '.*' (match 0 or more characters)
...
class OSCAddressSpace:
	...
	def dispatchMessage(self, pattern, tags, data, client_address):
		...
		for addr in list(self.callbacks.keys()):
			match = expr.match(addr.encode('utf8'))
			if match and (match.end() == len(addr)):
